...
final class TaskResource extends Resource
{
    ...
    {
        return __('resources.models.task.plural');
    }

    public static function getModelLabel(): string
    {
        return __('resources.models.task.singular');
    }
        ...
        return $table
            ...
            ->filters([
                Filter::make('assigned_to_me')
                    ->label(__('resources.filters.assigned_to_me'))
                                ...
                                if ($assignees->isNotEmpty()) {
                                    $assignees->each(function (User $recipient) use ($record): void {
                                        ...
                                        if (! $notificationExists) {
                                            Notification::make()
                                                ->title(__('resources.notifications.task_assignment', ['title' => $record->title]))
                                                ->actions([
                                                    Action::make('view')
                                                        ->button()
                                                        ->label(__('resources.actions.view_task'))
