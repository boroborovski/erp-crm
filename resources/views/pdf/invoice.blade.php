...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .badge-void    { background: #f3f4f6; color: #374151; }
        ...
        .totals .paid { color: #065f46; }
        .totals .outstanding { color: #991b1b; }

        .payments { margin-top: 32px; }
        .payments .section-label { font-size: 11px; font-weight: 600; text-transform: uppercase; color: #9ca3af; margin-bottom: 10px; letter-spacing: 0.05em; }
...
<div class="page">
    ...
    <div class="header">
        <div>
            <h1>{{ $invoice->team->name }}</h1>
        </div>
        <div class="meta">
            <div class="number">{{ $invoice->invoice_number }}</div>
            <span class="badge badge-{{ $invoice->status->value }}">{{ $invoice->status->getLabel() }}</span>
        </div>
    </div>
    ...
    <div class="parties">
        ...
        <div class="party">
            ...
            @if($invoice->contact)
            <div class="party-sub">{{ $invoice->contact->name }}</div>
            @endif
        </div>
        ...
        @if($invoice->quote)
        <div class="party">
            <div class="party-label">Quote Reference</div>
            <div class="party-name">{{ $invoice->quote->quote_number }}</div>
        </div>
        @endif
    </div>
    ...
    <div class="totals">
        <table>
            ...
            <tr class="outstanding">
                <td>Outstanding</td>
                <td class="right">{{ number_format($invoice->amount_outstanding, 2) }}</td>
            </tr>
            @endif
        </table>
    </div>
    ...
    <div class="footer">
        Generated by {{ $invoice->team->name }} &mdash; {{ now()->format('d M Y') }}
    </div>
